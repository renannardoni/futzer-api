...
def _expand_slot(s) -> list:
    """Converte slot legado (int hora cheia ou string HH:00) em 4 slots de 15 min."""
    if isinstance(s, int):
...
def _horarios_from_doc(raw) -> HorariosSemanais:
    """Retorna os slots SEMPRE em granularidade de 15 min (fonte de verdade).
    A discretizacaoMinima é aplicada apenas no frontend público."""
...
def _to_quadra(q: dict, include_reservas: bool = True) -> Quadra:
    ...
    discretizacao = q.get("discretizacaoMinima", q.get("discretizacao_minima")) or 15
    ...
    return Quadra(
        ...
        discretizacaoMinima=discretizacao,
